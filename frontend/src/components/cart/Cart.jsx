...
const Cart = ({ setOpenCart }) => {
    ...
    const {panier, products, getPanier} = useStateContext();
    const [cardProducts, setCardProducts] = useState([]);

    const getPanierProduits = _ => {
        setCardProducts(panier.map(ele => products.find(item => item.id == ele.id_produit)))
    }
    ...
    return (
        ...
        <div className="fixed top-0 right-0 min-h-full w-[25%] bg-white flex flex-col justify-between shadow-sm">
            ...
            <div className="w-full border-t">
                {cardProducts &&
                cardProducts.map((i, index) =>
                    <CartSingle total={total} setTotal={() => setTotal}  qtt={panier.find(ele => ele.id_produit == i.id)} key={index} data={i} />
                )}
            </div>
            </div>
    ...
    return (
        ...
        <div className="w-full flex items-center">
            ...
            <img src={data.image_produit} alt=""
